...
public class MLGenreTopMoviesNaive {
	...
	public static void main(String[] args) {
		...
		JavaPairRDD<String,String> movieGenres = movieData.flatMapToPair(s->{
			...
			if (length >=3){ // the genre data is present
		...
		movieGenres.coalesce(1);
		numRatingPerMovie.coalesce(1);
		...
		JavaPairRDD<String, MovieRatingCount> genreMovieRating =
				joinResults.values().mapToPair(v->{
					...
					return new Tuple2<String,MovieRatingCount>(titelGenre[1],
		...
		JavaPairRDD<String, LinkedList<MovieRatingCount>> genreTop5 = genreMovies.mapToPair(
				(t)->{
					...
					return new Tuple2<String, LinkedList<MovieRatingCount>>(t._1,results);
				}
				);
